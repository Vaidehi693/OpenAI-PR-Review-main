...
public class CheckoutController extends AbstractCheckoutController
{
	...
	protected String processRegisterGuestUserRequest(final GuestRegisterForm form, final BindingResult bindingResult,
		...
		{
			return handlePasswordPolicyViolationException(model, form, redirectModel, "registration.error.password.violate.policy");
		}
		...
		if (cookie != null)
		{
			...
			{
				LOG.error(String.format("Cookie Data could not be decoded : %s", Sanitizer.sanitize(cookie.getValue())), e);
			}
	...
	protected void processEmailAddress(final Model model, final OrderData orderDetails)
	{
		...
		{
			email = orderDetails.getPaymentInfo().getBillingAddress().getEmail();
			if (!model.containsAttribute("guestRegisterForm"))
			{
				final GuestRegisterForm guestRegisterForm = new GuestRegisterForm();
				guestRegisterForm.setOrderCode(orderDetails.getGuid());

				guestRegisterForm.setUid(email);
				model.addAttribute(guestRegisterForm);
			}
		}
		else
		{
			email = getCustomerFacade().getUserForUID(orderDetails.getUser().getUid()).getDisplayUid();
		}
	...
	private String handlePasswordPolicyViolationException(final Model model, final GuestRegisterForm form,
			final RedirectAttributes redirectModel, final String errorMessage) throws UnsupportedEncodingException
	{
		form.setTermsCheck(false);
		model.addAttribute(new GuestRegisterForm());
