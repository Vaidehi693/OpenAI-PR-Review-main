...
import com.hybris.OpenAIPrReview.storefront.controllers.ControllerConstants;
...
public class ProductPageController extends AbstractPageController
{
			...
			throws CMSItemNotFoundException, UnsupportedEncodingException
	{
		...
		final List<ProductOption> options = getProductOptionsForDisplay();
		...
		final var url = StringUtils.isNotEmpty(productData.getUrl()) ? productData.getUrl() : sapProductCodeUrlResolver.resolve(productCode);
		final String redirection = checkRequestUrl(request, response, url);
		...
		populateProductDetailForDisplay(productModel, productData, model, request);
	...
	private void populateProductDetailForDisplay(final ProductModel productModel, final ProductData productData, final Model model, final HttpServletRequest request) throws CMSItemNotFoundException
	{
	...
	private List<ProductOption> getProductOptionsForDisplay()
	{
		return new ArrayList<>(Arrays.asList(ProductOption.VARIANT_FIRST_VARIANT, ProductOption.BASIC,
	...
	protected AbstractPageModel getPageForProduct(final ProductModel productModel) throws CMSItemNotFoundException
	{
		return cmsPageService.getPageForProduct(productModel, getCmsPreviewService().getPagePreviewCriteria());
	}
