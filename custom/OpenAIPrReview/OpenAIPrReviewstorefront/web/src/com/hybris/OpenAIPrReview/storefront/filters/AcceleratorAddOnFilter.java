 ...
 */
package com.hybris.OpenAIPrReview.storefront.filters;
...
public class AcceleratorAddOnFilter extends GenericFilterBean
{
			...
			ServletException
	{
		...
		{
			LOG.debug("Hit by URI  [" + Sanitizer.sanitize(request.getRequestURI()) + "]");
		}
		if (isActive())
		{
			for (final ResourceAddOnResolver resolver : getAddOnResolvers())
			{
				...
				{
					final String sourceAddOnResource = DOT_PATTERN.matcher(resolver.getAddOnFullPathName(request)).replaceAll(DOT);
					final String targetAddOnResource = DOT_PATTERN.matcher(resolver.getStorefrontFullPathName(request))
							.replaceAll(DOT);
			...
			throws IOException
	{
		...
		{
			LOG.warn("Add-on source file [" + Sanitizer.sanitize(sourceAddOnFileName) + "] should exists ");
			return;
		}
		if (!targetAddOnFile.exists())
		{
			...
			{
				LOG.info("Unable to create addon folder for resource " + Sanitizer.sanitize(targetAddOnFile.getParent())
		...
		else
		{
			...
			{
				LOG.info("Copying <<" + Sanitizer.sanitize(sourceAddOnFile.getAbsolutePath()) + ">> to <<"
						+ Sanitizer.sanitize(targetAddOnFile.getAbsolutePath()) + ">>.");
				FileUtils.copyFile(sourceAddOnFile, targetAddOnFile);
			}
		}
	}
