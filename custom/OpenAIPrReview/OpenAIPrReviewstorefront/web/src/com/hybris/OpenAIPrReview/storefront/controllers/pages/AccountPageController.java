...
public class AccountPageController extends AbstractSearchPageController
{
			...
			final RedirectAttributes redirectModel)
	{
		...
		catch (final ModelNotFoundException | AmbiguousIdentifierException e)
		{
			LOG.warn(String.format("ConsentTemplate with code [%s] and version [%s] was not found",
					Sanitizer.sanitize(consentTemplateId), Sanitizer.sanitize(version.toString())), e);
			GlobalMessages.addFlashMessage(redirectModel, GlobalMessages.ERROR_MESSAGES_HOLDER,
					TEXT_ACCOUNT_CONSENT_TEMPLATE_NOT_FOUND, null);
		}
		catch (final CommerceConsentGivenException e)
		{
			LOG.warn(String.format("ConsentTemplate with code [%s] and version [%s] already has a given consent",
					Sanitizer.sanitize(consentTemplateId), Sanitizer.sanitize(version.toString())), e);
			...
			throws CMSItemNotFoundException
	{
		...
		{
			LOG.warn(String.format("Consent with code [%s] was not found", Sanitizer.sanitize(consentCode)), e);
		...
		{
			LOG.error(String.format("Consent with code [%s] is already withdrawn", Sanitizer.sanitize(consentCode)), e);
			GlobalMessages.addFlashMessage(redirectModel, GlobalMessages.ERROR_MESSAGES_HOLDER,
