...
import com.hybris.OpenAIPrReview.storefront.web.wrappers.UrlEncodeHttpRequestWrapper;
...
public class UrlEncoderFilter extends OncePerRequestFilter
{
			...
			final FilterChain filterChain) throws ServletException, IOException
	{
		...
		{
			LOG.debug(" The incoming URL : [" + Sanitizer.sanitize(request.getRequestURL().toString()) + "]");
		}
		...
		if (currentUrlEncoderDatas != null && !currentUrlEncoderDatas.isEmpty())
		{
			...
			if (LOG.isDebugEnabled())
			{
				LOG.debug("ContextPath=[" + Sanitizer.sanitize(wrappedRequest.getContextPath()) + "]" + " Servlet Path= ["
						+ Sanitizer.sanitize(wrappedRequest.getServletPath()) + "]" + " Request Url= ["
						+ Sanitizer.sanitize(wrappedRequest.getRequestURL().toString()) + "]");
			}
