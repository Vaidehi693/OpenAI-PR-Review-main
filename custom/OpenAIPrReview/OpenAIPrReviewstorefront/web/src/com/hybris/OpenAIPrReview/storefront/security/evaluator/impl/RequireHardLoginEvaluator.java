 ...
 */
package com.hybris.OpenAIPrReview.storefront.security.evaluator.impl;

import de.hybris.platform.acceleratorstorefrontcommons.constants.WebConstants;
import de.hybris.platform.order.CartService;
import de.hybris.platform.servicelayer.session.SessionService;
import de.hybris.platform.servicelayer.user.UserService;
import com.hybris.OpenAIPrReview.storefront.security.evaluator.SecurityTraitEvaluator;
			...
			final String guid, final String guidCookieName)
	{
		for (final Cookie cookie : request.getCookies())
		{
			if (guidCookieName.equals(cookie.getName()))
			{
				...
				{
					LOG.info("Found secure cookie with invalid value. expected [" + Sanitizer.sanitize(guid) + "] actual ["
							+ Sanitizer.sanitize(cookie.getValue()) + "]. removing.");
